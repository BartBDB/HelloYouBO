import os

print("Dit is mijn eindproject: de textbased applicatie in Python.")
print("Dit keuzeverhaal gaat over Wahhab, uit Irak.")
print("Door spanningen in Irak is er een oorlog uitgebroken, en kon Wahhab zichzelf nietmeer zijn.")
print("Het verhaal start vlak voordat hij gaat vluchten.")
print("Nog belangrijk: als er geen geldig antwoord word gegeven, gaat het programma altijd uit van de tweede keuze. Let dus op dat je het goed invult.")
print()
print("Wahhab wordt wakker, en gaat op zijn tempo naar beneden om wat te gaan drinken en om TV te kijken.")
print("Tijdens het nieuws hoort hij dat in zijn woonplaats de politie is binnengevallen om Jezidi's te zoeken.") 
print("Wat deed hij nadat hij dat hoorde?")
print("A - Wahhab ging naar zijn buren toe, om te vragen of zij er meer vanaf wisten.")
print("B - Wahhab ging snel zijn spullen pakken om te vluchten, om enige confrontatie te voorkomen.")
vraag1 = input("A of B? ")
if vraag1 == ("A") or ("a"):
    os.system("cls")
    print("Wahhab besloot om naar zijn buren te gaan en om te vragen of zij meer wisten.")
    print("Jammer genoeg wisten ze er zelf redelijk weinig van, al vertrouwde Wahhab het niet helemaal.")
    print("A - Wahhab bleef doorvragen.")
    print("B - Wahhab ging terug naar zijn huis, om spullen te pakken en te vluchten.")
    vraag2 = input("A of B? ")
    if vraag2 == ("A") or ("a"):
        os.system("cls")
        print("Wahhab bleef doorvragen.")
        print("Het blijkt dus dat zijn buren hem hadden verraden, voor een grote som geld.")
        print("Ze hadden ook een lijn openstaan voor contact met de politie.")
        print("Toen Wahhab snel wou wegrennen, stond de politie al voor de deur.")
        print("Dit is dus het einde van het verhaal. Wahhab zit vast in de cel en kan er voorlopig ook niet uitkomen.")
        print("Ik kan weinig doen nu voor zijn familie. Helaas gebeurt dit vaker")
        print("Dossier gesloten.")
    else:
        os.system("cls")
        print("Wahhab ging snel naar huis. Hij kon veel spullen pakken, maar hij hoorde de sirenes, dus het was een slimmer idee om op de voet te gaan.")
        print("Waar ging Wahhab eerst heen?")
        vraag4 = input("A of B? ")
        print("A - Zijn broer")
        print("B - Zijn ouders")
        if vraag4 == ("A") or ("a"):
            os.system("cls")
            print("Wahhab kwam bij zijn broer aan, die ook een Jezidi is.")
            print("Samen besloten ze om naar Turkije te vluchten, en vanaf daar een vliegtuig naar een veilig land te nemen")
            print("Ze stapten in de auto van de broer, en reden weg.")
            print("Naarmaate het later werd hadden ze een plek nodig om te slapen, waar sliepen ze?")
            print("A - In de auto.")
            print("B - In een goedkoop hotel.")
            #Hier komt hetzelfde als vraag 3, simpel copy-paste dus
            vraag5 = input("A of B? ")
            if vraag5 == ("A") or ("a"):
                os.system("cls")
                print("Ze besloten dat het veiliger was om in de auto te slapen, op een donkere plek")
                print("Gelukkig maar, want blijkbaar waren de hotels uitgekamt, en waren andere Jezidi's opgepakt.")
                print("Na een nacht rust kwamen ze eindelijk aan bij Turkije. Eindelijk veilig van de politie.")
                print("Maar goed, nu komt nog een belangrijk punt: naar welk land vluchten ze?")
                print("A - Duitsland")
                print("B - Nederland")
                vraag6 = input("A of B? ")
                if vraag6 == ("A") or ("a"):
                    os.system("cls")
                    print("De broers besloten om naar Duitsland te vluchten.")
                    print("Na een redelijk korte vlucht komen ze in Duitsland aan.")
                    print("Er was alleen 1 probleem: het is behoorlijk lastig om als vluchteling in Duitsland in te burgeren.")
                    print("Ze kwamen uiteindelijk bij mij aan, in het asielzoekerscentrum.")
                    print("Ze vroegen of ik wat voor hun familie kon doen. Zou ik proberen om te bellen?")
                    print("A - Probeer om te bellen.")
                    print("B - Bewaar het voor later.")
                    vraag7 = input("A of B? ")
                    if vraag7 == ("A") or ("a"):
                        os.system("cls")
                        print("Ik probeerde dus om te bellen, om te zien of ik iets kon betekenen voor ze.")
                        print("Gelukkig was ik net op tijd met bellen, en kon ik ervoor zorgen dat ze naar de grens kwamen.")
                        print("Helaas werden ze door de overheid weer teruggestuurd naar Irak.")
                        print("De broers waren teleurgesteld, want waarschijnlijk kon het in Nederland wel.")
                        print("Dit is dus alles wat ik helaas voor ze kon doen, alhoewel ze nu wel samen in een huis wonen.")
                        print("Dossier gesloten.")
                    else:
                        os.system("cls")
                        print("Ik besloot om later te bellen, omdat er belangrijkere zaken waren waaraan ik moest gaan werken.")
                        print("Ik belde de volgende dag, en het bleek dat de familie vast zat in de cel.")
                        print("Ik kan helaas hier niets voor ze doen.")
                        print("De broers waren boos op mij geworden omdat ik later belde, zelfs nadat ik had gezegd dat er belangrijkere zaken waren die ik moest doen.")
                        print("Ze zijn uiteindelijk zelf teruggevlogen naar Irak om ze uit de gevangenis proberen te krijgen.")
                        print("Ik heb niet meer van hun gehoord.")
                        print("Dossier gesloten")
                else:
                    os.system("cls")
                    print("De broers besloten om naar Nederland te gaan.")
                    print("Goede keus, want Nederland is meestal best wel soepel wanneer het komt tot inburgeren.")
                    print("Na een lange tijd wachten in het gebouw kwamen ze bij mij aan, de begeleider.")
                    print("Ik heb met ze besproken hoe het vanaf hier verder gaat.")
                    print("Ze willen uiteindelijk dus een veilige plek om te wonen.")
                    print("Er was ook gevraagd of ik hun familie kan halen uit Irak.")
                    print("Wie zal ik gaan bellen daarvoor?")
                    print("A - De politie van Irak")
                    print("B - De overheid van Irak")
                    vraag8 = input("A of B? ")
                    if vraag8 == ("A") or ("a"):
                        os.system("cls")
                        print("Ik had geprobeerd om de politie in Irak te bellen.")
                        print("Ik had het eigenlijk al een beetje verwacht, maar zij konden niets voor mij doen.")
                        print("Wat ik niet had verwacht, was toen ik de overheid daar had gebeld, dat er werd gezegd dat ze er niet meer uit konden.")
                        print("Ik had het slechte nieuws verteld. Ze waren boos, maar begrepen het wel.")
                        print("Na een paar weken kregen ze de brief binnen waarin stond dat ze hier mochten blijven.")
                        print("Nu wonen ze dus hier, zonder familie helaas.")
                        print("Dossier gesloten.")
                    else:
                        os.system("cls")
                        print("Ik had geprobeerd om de overheid in Irak te bellen.")
                        print("Gelukkig was ik op tijd met bellen en kon ik ze op tijd waarschuwen om te vluchten.")
                        print("Waar stuurde ik ze heen?")
                        print("A - Zo snel mogelijk naar Turkije, met de auto.")
                        print("B - Op de voet, zo snel mogelijk naar de grens.")
                        vraag9 = input("A of B? ")
                        if vraag9 == ("A") or ("a"):
                            os.system("cls")
                            print("Ze konden een auto regelen, dus ze reden zo snel mogelijk naar de grens.")
                            print("Gelukkig was het veilig genoeg, dus kwamen ze aan zonder problemen.")
                            print("Na een paar dagen waren ze ook in Nederland.")
                            print("Na een korte inburgering waren ze ook burgers in Nederland.")
                            print("Een happy ending dus, mooi.")
                            print("Dossier gesloten.")
                        else:
                            os.system("cls")
                            print("Ze konden geen auto regelen, dus zei ik dat ze zo snel mogelijk op de voet moesten komen.")
                            print("Uiteindelijk waren ze heel dichtbij de grens gekomen.")
                            print("Jammer genoeg waren ze niet snel genoeg, en werden ze opgepakt.")
                            print("Toen ik het slechte nieuws vertelde tegen de broers waren ze boos, maar ze bergrepen wel dat ik er niets tegen kon doen.")
                            print("Uiteindelijk hebben de broers wel een huis hier kunnen krijgen, dus dat is wel top.")
                            print("Dossier gesloten.")
            else:
                os.system("cls")
                print("Ze besloten om in een goedkoop hotel te gaan slapen")
                print("Jammer genoeg werden alle hotels in de buurt uitgekamt, dus werden ze gevonden door de politie.")
                print("Ze zitten nu vast in de cel, en komen er voorlopig niet uit.")
                print("Ik kan helaas niets doen voor de familie, dit soort zaken gebeuren vaker.")
                print("Dossier gesloten.") 
        else:
            os.system("cls")
            print("Wahhab ging naar zijn ouders toe voor advies, al waren ze zelf geen Jezidi")
            print("Gelukkig wisten ze wel wat er gaande was, dus konden ze Wahhab helpen")
            print("Ze gingen Wahhab helpen om bij de grens te komen, waar hij zelf wel zijn gang moet gaan")
            print("Eenmaal in Turkije zou hij een vliegtuig kunnen nemen naar een ander land om veilig te zijn.")
            print("Hij kon veilig gaan rijden vanaf dat punt, dus leende hij de auto van zijn ouders om veilig weg te komen.")
            print("Dat is wat zijn broer had verteld.")
            print("Helaas kon ik hem niet meer bereiken, dus weet ik niet meer wat ik moet doen.")
            print("Dossier gesloten.")
            
            
else:
    os.system("cls")
    print("Wahhab pakte gauw alle spullen die hij nodig had: tandenborstel, kleding, portemonee, etc")
    print("Wahhab stapte in zijn auto en reed snel weg")
    print("Reed hij eerst langs zijn broer om hem mee te nemen of ging hij alleen?")
    print("A - Hij nam zijn broer mee")
    print("B - Hij ging alleen")
    vraag3 = input("A of B? ")
    if vraag3 == ("A") or ("a"):
        os.system("cls")
        print("Wahhab kwam bij zijn broer aan, die ook een Jezidi is.")
        print("Samen besloten ze om naar Turkije te vluchten, en vanaf daar een vliegtuig naar een veilig land te nemen")
        print("Ze stapten in de auto van de broer, en reden weg.")
        print("Naarmaate het later werd hadden ze een plek nodig om te slapen, waar sliepen ze?")
        print("A - In de auto.")
        print("B - In een goedkoop hotel.")
        vraag5 = input("A of B? ")
        if vraag5 == ("A") or ("a"):
            os.system("cls")
            print("Ze besloten dat het veiliger was om in de auto te slapen, op een donkere plek")
            print("Gelukkig maar, want blijkbaar waren de hotels uitgekamt, en waren andere Jezidi's opgepakt.")
            print("Na een nacht rust kwamen ze eindelijk aan bij Turkije. Eindelijk veilig van de politie.")
            print("Maar goed, nu komt nog een belangrijk punt: naar welk land vluchten ze?")
            print("A - Duitsland")
            print("B - Nederland")
            vraag6 = input("A of B? ")
            if vraag6 == ("A") or ("a"):
                os.system("cls")
                print("De broers besloten om naar Duitsland te vluchten.")
                print("Na een redelijk korte vlucht komen ze in Duitsland aan.")
                print("Er was alleen 1 probleem: het is behoorlijk lastig om als vluchteling in Duitsland in te burgeren.")
                print("Ze kwamen uiteindelijk bij mij aan, in het asielzoekerscentrum.")
                print("Ze vroegen of ik wat voor hun familie kon doen. Zou ik proberen om te bellen?")
                print("A - Probeer om te bellen.")
                print("B - Bewaar het voor later.")
                vraag7 = input("A of B? ")
                if vraag7 == ("A") or ("a"):
                    os.system("cls")
                    print("Ik probeerde dus om te bellen, om te zien of ik iets kon betekenen voor ze.")
                    print("Gelukkig was ik net op tijd met bellen, en kon ik ervoor zorgen dat ze naar de grens kwamen.")
                    print("Helaas werden ze door de overheid weer teruggestuurd naar Irak.")
                    print("De broers waren teleurgesteld, want waarschijnlijk kon het in Nederland wel.")
                    print("Dit is dus alles wat ik helaas voor ze kon doen, alhoewel ze nu wel samen in een huis wonen.")
                    print("Dossier gesloten.")
                else:
                    os.system("cls")
                    print("Ik besloot om later te bellen, omdat er belangrijkere zaken waren waaraan ik moest gaan werken.")
                    print("Ik belde de volgende dag, en het bleek dat de familie vast zat in de cel.")
                    print("Ik kan helaas hier niets voor ze doen.")
                    print("De broers waren boos op mij geworden omdat ik later belde, zelfs nadat ik had gezegd dat er belangrijkere zaken waren die ik moest doen.")
                    print("Ze zijn uiteindelijk zelf teruggevlogen naar Irak om ze uit de gevangenis proberen te krijgen.")
                    print("Ik heb niet meer van hun gehoord.")
                    print("Dossier gesloten")
            else:
                os.system("cls")
                print("De broers besloten om naar Nederland te gaan.")
                print("Goede keus, want Nederland is meestal best wel soepel wanneer het komt tot inburgeren.")
                print("Na een lange tijd wachten in het gebouw kwamen ze bij mij aan, de begeleider.")
                print("Ik heb met ze besproken hoe het vanaf hier verder gaat.")
                print("Ze willen uiteindelijk dus een veilige plek om te wonen.")
                print("Er was ook gevraagd of ik hun familie kan halen uit Irak.")
                print("Wie zal ik gaan bellen daarvoor?")
                print("A - De politie van Irak")
                print("B - De overheid van Irak")
                vraag8 = input("A of B? ")
                if vraag8 == ("A") or ("a"):
                    os.system("cls")
                    print("Ik had geprobeerd om de politie in Irak te bellen.")
                    print("Ik had het eigenlijk al een beetje verwacht, maar zij konden niets voor mij doen.")
                    print("Wat ik niet had verwacht, was toen ik de overheid daar had gebeld, dat er werd gezegd dat ze er niet meer uit konden.")
                    print("Ik had het slechte nieuws verteld. Ze waren boos, maar begrepen het wel.")
                    print("Na een paar weken kregen ze de brief binnen waarin stond dat ze hier mochten blijven.")
                    print("Nu wonen ze dus hier, zonder familie helaas.")
                    print("Dossier gesloten.")
                else:
                    os.system("cls")
                    print("Ik had geprobeerd om de overheid in Irak te bellen.")
                    print("Gelukkig was ik op tijd met bellen en kon ik ze op tijd waarschuwen om te vluchten.")
                    print("Waar stuurde ik ze heen?")
                    print("A - Zo snel mogelijk naar Turkije, met de auto.")
                    print("B - Op de voet, zo snel mogelijk naar de grens.")
                    vraag9 = input("A of B? ")
                    if vraag9 == ("A") or ("a"):
                        os.system("cls")
                        print("Ze konden een auto regelen, dus ze reden zo snel mogelijk naar de grens.")
                        print("Gelukkig was het veilig genoeg, dus kwamen ze aan zonder problemen.")
                        print("Na een paar dagen waren ze ook in Nederland.")
                        print("Na een korte inburgering waren ze ook burgers in Nederland.")
                        print("Een happy ending dus, mooi.")
                        print("Dossier gesloten.")
                    else:
                        os.system("cls")
                        print("Ze konden geen auto regelen, dus zei ik dat ze zo snel mogelijk op de voet moesten komen.")
                        print("Uiteindelijk waren ze heel dichtbij de grens gekomen.")
                        print("Jammer genoeg waren ze niet snel genoeg, en werden ze opgepakt.")
                        print("Toen ik het slechte nieuws vertelde tegen de broers waren ze boos, maar ze bergrepen wel dat ik er niets tegen kon doen.")
                        print("Uiteindelijk hebben de broers wel een huis hier kunnen krijgen, dus dat is wel top.")
                        print("Dossier gesloten.")
        else:
            os.system("cls")
            print("Ze besloten om in een goedkoop hotel te gaan slapen")
            print("Jammer genoeg werden alle hotels in de buurt uitgekamt, dus werden ze gevonden door de politie.")
            print("Ze zitten nu vast in de cel, en komen er voorlopig niet uit.")
            print("Ik kan helaas niets doen voor de familie, dit soort zaken gebeuren vaker.")
            print("Dossier gesloten.")       
    else:
        os.system("cls")
        print("Wahhab besloot om alleen te gaan, omdat dat net iets veiliger zou zijn.")
        print("Het enigste probleem was dat Wahhab niet heel goed was met richtingen.")
        print("Waar ging hij als eerste heen?")
        print("A - Hij vroeg de weg aan een willekeurig persoon op straat.")
        print("B - Hij loopt in een winkel en vraagt daar voor de weg.")
        vraag10 = input("A of B? ")
        if vraag10 == ("A") or ("a"):
            os.system("cls")
            print("Wahhab vroeg de weg aan een man op de straat.")
            print("De man kon hem gelukkig helpen, en Wahhab ging snel verder rijden.")
            print("Helaas bleek het dat de man hem de verkeerde kant op had gestuurd.")
            print("Door dit probleem zit hij nu zonder benzine, en zonder geld om benzine te kopen.")
            print("Wat moet Wahhab nu gaan doen?")
            print("A - Op de voet naar de grens, heel ver kan het niet zijn.")
            print("B - Probeer via een bus bij de grens te komen.")
            vraag11 = input("A of B? ")
            if vraag11 == ("A") or ("a"):
                os.system("cls")
                print("Hij besloot om op de voet verder te gaan.")
                print("Gelukkig voor hem kwam hij veilig aan bij de grens, en omdat hij geen extra geld had besteed kon hij een kaartje naar Nederland kopen.")
                print("Hij had mij alles verteld, toen hij hier was.")
                print("Hij heeft geluk gehad, meeste mensen redden het niet te voet.")
                print("Nu zit hij in een veilig huis hier in Nederland.")
                print("Dossier gesloten.")
            else:
                os.system("cls")
                print("Wahhab besloot om te proberen met een bus daar te komen.")
                print("Er was een onverwachte controle tijdens de rit, en hij werd daar geforceerd om te betalen.")
                print("Toen hij aankwam bij Turkije had hij niet genoeg geld om de vliegticket te betalen.")
                print("Dat is wat zijn broer tegen mij zij.")
                print("Nu probeer ik geld naar hem te krijgen zodat hij hierheen kan komen.")
                print("Het komt dus goed, al zit hij nu dakloos in Turkije.")
                print("Dossier gesloten.")
        else:
            os.system("cls")
            print("Wahhab ging een winkel in om de weg te vragen.")
            print("Gelukkig werd hij makkelijk geholpen, en krijg hij goede richtingen.")
            print("Waar ging Wahhab heen?")
            print("A - Naar het noorden, een snellere route maar de grens is meer bewaakt.")
            print("B - Naar het noordoosten, een langere route maar de grens is makkelijker om over te steken.")
            vraag12 = input("A of B? ")
            if vraag12 == ("A") or ("a"):
                os.system("cls")
                print("Wahhab besloot om naar het noorden te gaan.")
                print("Hij kwam aan bij de grens, maar hij kong er zelf niet doorkomen in zijn auto, omdat het bewaakt was.")
                print("Hoe ging hij verder?")
                print("A - Via een gat in het hek.")
                print("B - Hij liftte met een vrachtwagen.")
                vraag13 = input("A of B? ")
                if vraag13 == ("A") or ("a"):
                    os.system("cls")
                    print("Wahhab ging door een gat via het hek over de grens.")
                    print("Hij rende snel weg nadat hij door het gat was gegaan, en was gelukkig niet gespot door bewaking.")
                    print("Daarna nam hij snel een vliegtuig naar Nederland, en kwam hij bij het asielzoekerscentrum aan.")
                    print("Nu zit hij in een huis, te wachten tot zijn familie ook vlucht.")
                    print("Dossier gesloten.")
                else:
                    os.system("cls")
                    print("Wahhab besloot om te liften met een vrachtwagen.")
                    print("Hij had geluk dat de grenscontrole hem niet zag toen ze de vrachtwagen aan het controleren waren.")
                    print("Het enigste probleem is dat hij nu nog een paar dagen vastzit in Turkije, omdat hij niet genoeg geld heeft voor een ticket.")
                    print("Zijn broer zei dat tegen mij, toen hij bij het asielzoekerscentrum aankwam.")
                    print("Als het goed is zou Wahhab binnen een paar dagen aan moeten kunnen komen, maar voor nu is het dossier gesloten.")
            else:
                os.system("cls")
                print("Wahhab besloot om naar het noordoosten te gaan.")
                print("De rit duurde iets langer, maar hij had geluk dat er geen controles waren.")
                print("Maar goed, hij is nu bij de grens. Wat gaat hij doen?")
                print("A - Door een gat in het hek")
                print("B - Met de auto over de grens rijden, en hopen dat er geen controle is.")
                vraag14 = input("A of B? ")
                if vraag14 == ("A") or ("a"):
                    os.system("cls")
                    print("Hij besloot om door een gat in het hek te gaan.")
                    print("Hij kwam er veilig doorheen en over de grens, maar nu moet hij nog bij het vliegveld komen.")
                    print("Hoe kwam hij daar?")
                    print("A - Lopend")
                    print("B - Liften met een vrachtwagen")
                    vraag15 = input("A of B? ")
                    if vraag15 == ("A") or ("a"):
                        os.system("cls")
                        print("Wahhab besloot om te lopen.")
                        print("Het duurde even, maar hij kwam veilig bij het vliegveld aan.")
                        print("Dat is het verhaal wat zijn broer aan mij heeft verteld, en nu wonen ze veilig hier in Nederland.")
                        print("Dossier gesloten.")
                    else:
                        os.system("cls")
                        print("Hij besloot om te liften.")
                        print("Hij moest wel betalen, maar hij kwam veilig aan.")
                        print("Daardoor zit hij nu nog een paar dagen vast in Turkije, maar hij komt in ieder geval veilig aan.")
                        print("Dossier gesloten.")
                else:
                    os.system("cls")
                    print("Hij probeerde om met de auto over de grens te komen.")
                    print("Helaas was er wel controle, maar hij werd alleen teruggestuurd.")
                    print("Hij besloot om met zijn andere optie te gaan: door een gat in het hek gaan.")
                    print("Hij kwam er veilig doorheen en over de grens, maar nu moet hij nog bij het vliegveld komen.")
                    print("Hoe kwam hij daar?")
                    print("A - Lopend")
                    print("B - Liften met een vrachtwagen")
                    vraag15 = input("A of B? ")
                    if vraag15 == ("A") or ("a"):
                        os.system("cls")
                        print("Wahhab besloot om te lopen.")
                        print("Het duurde even, maar hij kwam veilig bij het vliegveld aan.")
                        print("Dat is het verhaal wat zijn broer aan mij heeft verteld, en nu wonen ze veilig hier in Nederland.")
                        print("Dossier gesloten.")
                    else:
                        os.system("cls")
                        print("Hij besloot om te liften.")
                        print("Hij moest wel betalen, maar hijw kwam veilig aan.")
                        print("Daardoor zit hij nu nog een paar dagen vast in Turkije, maar hij komt in ieder geval veilig aan.")
                        print("Dossier gesloten.")
            